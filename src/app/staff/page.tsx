    ...
    <>
      ...
      <div className="content">
        <section className="panel">
          <div className="section-head"><h2>Role Permissions</h2></div>
          <div className="grid cols-2">
            <div className="card compact-card"><strong>Super Admin</strong><p className="muted">Accesses every location, manages users, edits or deletes configuration, adjusts payments, and controls system settings.</p></div>
            <div className="card compact-card"><strong>Community Host</strong><p className="muted">Works only inside the assigned location. Can manage daily operations and configure local pass types, coffee menu, room types, and rooms, but cannot manage users.</p></div>
          </div>
        </section>
...
function StaffForm({ user, locationId, locations }: { user?: any; locationId?: string; locations: any[] }) {
  return (
    <form action={upsertStaff} className="form-grid">
      ...
      <div className="field"><label>Role</label><select name="role" defaultValue={user?.role ?? "ADMIN"}><option value="ADMIN">Community Host</option><option value="SUPER_ADMIN">Super Admin</option></select></div>
      <div className="field"><label>{user ? "New password" : "Password"}</label><input name="password" type="password" minLength={8} required={!user} /></div>
      <div className="field"><label>{user ? "Confirm new password" : "Confirm password"}</label><input name="confirmPassword" type="password" minLength={8} required={!user} /></div>
