...
export default async function DashboardPage() {
  ...
  const [todayCheckIns, inside, bookings, expiring, payments, sales, activities, creditMembers] = await Promise.all([
    ...
    prisma.customer.findMany({ where: { isInside: true }, orderBy: { fullName: "asc" } }),
    prisma.booking.findMany({
      where: { startsAt: { gte: todayStart, lte: todayEnd }, status: { in: ["PENDING", "CONFIRMED"] } },
      include: { customer: true, room: true },
      orderBy: { startsAt: "asc" },
      take: 8
    }),
    prisma.customer.findMany({
      where: { OR: [{ membershipExpiresAt: { lt: new Date() } }, { membershipExpiresAt: { gte: new Date(), lte: soon } }, { remainingCoworkingDays: { lte: 1 } }] },
      orderBy: { membershipExpiresAt: "asc" },
      take: 8
    }),
    prisma.payment.findMany({ where: { status: { in: ["UNPAID", "PARTIALLY_PAID"] } }, include: { customer: true }, orderBy: { paymentDate: "desc" }, take: 8 }),
    prisma.payment.groupBy({
      by: ["paymentFor"],
      where: { paymentDate: { gte: todayStart, lte: todayEnd }, status: { in: ["PAID", "PARTIALLY_PAID"] } },
      _sum: { amount: true }
    }),
    prisma.activityLog.findMany({ orderBy: { createdAt: "desc" }, take: 8, include: { staff: true } }),
    prisma.customer.findMany({ where: { remainingMeetingCreditHours: { gt: 0 } }, orderBy: { remainingMeetingCreditHours: "desc" }, take: 6 })
  ]);
  ...
  return (
    <>
      <PageHeader
        title="Dashboard"
        subtitle="Today’s check-ins, bookings, renewals, payments, and sales at a glance."
        action={
